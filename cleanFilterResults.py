...
def clean_level_of_study(value):
    ...
    value = re.sub(r'Recent Graduatess', 'Recent Graduates', str(value), flags=re.IGNORECASE)
    return value
...
def clean_opporunities(value):
    value = re.sub(r'Analysts', 'Analyst', str(value), flags=re.IGNORECASE)
    value = re.sub(r'Developers', 'Developer', str(value), flags=re.IGNORECASE)
...
df['Opportunities'] = df['Opportunities'].apply(clean_opporunities)
